...
- **Known non-goal:** multi-account provider runs write one output tree per account but produce one run manifest per account rather than a merged one. That matches the TUI's per-account behavior and is not a regression.
