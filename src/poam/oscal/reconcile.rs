...
mod tests {
    ...
    fn first_run_with_no_baseline_appends_all_as_new() {
        let triples = vec![build_inspector2_triple(
            &finding("k1", "ACTIVE"),
            "2026-01-01T00:00:00Z",
        )];
        let (doc, added, closed) =
            reconcile_document(None, triples, "Test POA&M", "2026-01-01T00:00:00Z");
        assert_eq!(added, 1);
        assert_eq!(closed, 0);
    ...
    fn second_run_same_finding_keeps_same_uuid_no_duplicate() {
        ...
        let (baseline, _, _) =
            reconcile_document(None, vec![triple1], "Test POA&M", "2026-01-01T00:00:00Z");
        ...
        let (doc2, added, closed) = reconcile_document(
            Some(baseline),
            vec![triple2],
            "Test POA&M",
        ...
        assert_eq!(
            added, 0,
            "finding already present in baseline should not count as newly added"
        );
        assert_eq!(closed, 0);
        assert_eq!(
            doc2.poam_items.len(),
            1,
            "same stable key must not produce a duplicate item"
        );
        assert_eq!(
            doc2.poam_items[0].uuid, original_uuid,
            "uuid must be stable across runs"
        );
    }
    ...
    fn finding_absent_from_new_scan_transitions_to_closed_not_deleted() {
        ...
        let (doc2, added, closed) =
            reconcile_document(Some(baseline), vec![], "Test POA&M", "2026-02-01T00:00:00Z");
        ...
        assert_eq!(
            doc2.poam_items.len(),
            1,
            "closed item must remain in the document, not be deleted"
        );
    ...
    fn finding_reappearing_after_close_reopens_risk_status() {
        ...
        let (baseline2, _, closed) = reconcile_document(
            Some(baseline1),
            vec![],
            "Test POA&M",
            "2026-02-01T00:00:00Z",
        ...
        let (doc3, added, closed3) = reconcile_document(
            Some(baseline2),
            vec![triple3],
            "Test POA&M",
        ...
        assert_eq!(
            added, 0,
            "matched stable key must not be re-appended as a new finding"
        );
        assert_eq!(closed3, 0);
    ...
    fn finding_present_in_both_but_scanner_marks_it_closed_updates_risk_status() {
        ...
        assert_eq!(
            closed, 0,
            "this is an in-place status refresh, not the 'absent -> close' path"
        );
    ...
    fn custom_item_with_no_weakness_source_identifier_prop_survives_reconcile_untouched() {
        ...
        let (mut baseline, _, _) =
            reconcile_document(None, vec![triple], "Test POA&M", "2026-01-01T00:00:00Z");
        ...
        assert_eq!(
            doc2.poam_items.len(),
            custom_items_before,
            "custom item must not be removed"
        );
        let found_item = doc2
            .poam_items
            .iter()
