    ...
    async fn collect_rows(
        ...
        loop {
            ...
            for alarm in resp.metric_alarms() {
                ...
                let threshold = alarm.threshold().map(|t| t.to_string()).unwrap_or_default();
                let comparison = alarm
                    .comparison_operator()
                    .map(|c| c.as_str().to_string())
                    .unwrap_or_default();
                let actions_enabled = if alarm.actions_enabled().unwrap_or(false) {
                    "Yes"
                } else {
                    "No"
                }
    ...
    async fn collect_rows(
        ...
        loop {
            ...
            let resp = req
                .send()
                .await
                .context("CloudWatchLogs describe_log_groups")?;
            ...
            for lg in resp.log_groups() {
                ...
                let stored = lg.stored_bytes().map(|b| b.to_string()).unwrap_or_default();
