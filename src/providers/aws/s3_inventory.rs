...
impl CsvCollector for S3BucketLoggingCollector {
    ...
    async fn collect_rows(
        ...
        for bucket in buckets_resp.buckets() {
            ...
            // Region
            let region = match self.client.get_bucket_location().bucket(&name).send().await {
                Ok(r) => r
                    .location_constraint()
            ...
            // Encryption
            let (encrypted, enc_type, kms_key) = match self
                .client
            ...
            // Access logging
            let logging = match self.client.get_bucket_logging().bucket(&name).send().await {
                Ok(l) => if l.logging_enabled().is_some() {
                    "Enabled"
                } else {
                    "Disabled"
                }
