...
impl CsvCollector for CloudWatchConfigAlarmsCollector {
    fn name(&self) -> &str {
        "CloudWatch Alarms for Config Changes"
    }
    fn filename_prefix(&self) -> &str {
        "Change_Alerts_Config"
    }
        ...
        loop {
            ...
            for alarm in resp.metric_alarms() {
                ...
                let comparison = alarm
                    ...
                    .unwrap_or_default();
                let actions_enabled = alarm.actions_enabled().unwrap_or(false).to_string();
                let actions: Vec<&str> = alarm.alarm_actions().iter().map(|s| s.as_str()).collect();
