pub mod factory;

pub mod access_analyzer;
pub mod account_config;
pub mod acm;
pub mod acm_pca;
pub mod alb_logs;
pub mod ami_default_creds;
pub mod apigateway;
pub mod autoscaling;
pub mod backup;
pub mod backup_config;
pub mod client_vpn;
pub mod cloudformation_drift;
pub mod cloudfront;
pub mod cloudtrail;
pub mod cloudtrail_config;
pub mod cloudtrail_details;
pub mod cloudtrail_iam;
pub mod cloudtrail_inventory;
pub mod cloudtrail_s3;
pub mod cloudwatch;
pub mod cloudwatch_alarms;
pub mod cloudwatch_config;
pub mod cloudwatch_resources;
pub mod config_history;
pub mod config_rules;
pub mod config_timeline;
pub mod doc_repo_backup;
pub mod dynamodb;
pub mod ebs;
pub mod ec2_config;
pub mod ec2_detailed;
pub mod ec2_inventory;
pub mod ecr_config;
pub mod ecs;
pub mod efs;
pub mod eks;
pub mod elasticache;
pub mod elb;
pub mod elb_config;
pub mod guardduty;
pub mod guardduty_config;
pub mod guardduty_malware_scans;
pub mod guardduty_runtime;
pub mod iam_certs;
pub mod iam_credential_report;
pub mod iam_inventory;
pub mod iam_policies;
pub mod iam_trusts;
pub mod inspector;
pub mod inspector_config;
pub mod inspector_ecr;
pub mod inspector_history;
pub mod inspector_sbom;
pub mod kms;
pub mod kms_config;
pub mod kms_policies;
pub mod lambda_config;
pub mod launch_templates;
pub mod license_manager;
pub mod macie;
pub mod network_firewall;
pub mod network_firewall_failclosed;
pub mod network_gateways;
pub mod org_config;
pub mod organizations;
pub mod public_resources;
pub mod rds;
pub mod rds_inventory;
pub mod rds_snapshots;
pub mod route53_config;
pub mod s3_config;
pub mod s3_detail;
pub mod s3_inventory;
pub mod s3_policies;
pub mod secrets_extended;
pub mod secretsmanager;
pub mod security_svc_config;
pub mod securityhub;
pub mod securityhub_standards;
pub mod service_quotas;
pub mod session_timeouts;
pub mod shield;
pub mod sns;
pub mod sns_eventbridge;
pub mod ssm;
pub mod ssm_allowlist;
pub mod ssm_automation_runbooks;
pub mod ssm_extended;
pub mod ssm_patch_detail;
pub mod ssm_sessions;
pub mod ssm_software_inventory;
pub mod tagging_config;
pub mod transit_gateway_peering;
pub mod vpc;
pub mod vpc_endpoints;
pub mod vpcflowlogs;
pub mod waf;
pub mod waf_full_config;
pub mod waf_logging;
