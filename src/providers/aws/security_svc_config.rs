...
impl CsvCollector for GuardDutyFullConfigCollector {
    fn name(&self) -> &str {
        "GuardDuty Full Configuration"
    }
    fn filename_prefix(&self) -> &str {
        "GuardDuty_Full_Config"
    }
...
impl CsvCollector for AwsConfigRecorderCollector {
    ...
    async fn collect_rows(
        ...
        let recorders = match self.client.describe_configuration_recorders().send().await {
            Ok(r) => r,
            Err(e) => {
        ...
        for recorder in recorders.configuration_recorders() {
            ...
            let all_supported = recorder
                .recording_group()
                .map(|rg| rg.all_supported().to_string())
                .unwrap_or_default();
            let include_global = recorder
                .recording_group()
                .map(|rg| rg.include_global_resource_types().to_string())
                .unwrap_or_default();

            let (recording, last_status, last_change) = statuses
                .iter()
                .find(|(n, _, _, _)| n == &name)
