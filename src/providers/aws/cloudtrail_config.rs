...
impl CsvCollector for CloudTrailFullConfigCollector {
    ...
    async fn collect_rows(
        ...
        for trail in trails_resp.trail_list() {
            ...
            // Is logging status
            let is_logging = match self.client.get_trail_status().name(&trail_arn).send().await {
                Ok(r) => r.is_logging().unwrap_or(false).to_string(),
                Err(_) => "Unknown".to_string(),
            };

            // Event selectors
            let event_selectors = match self
                .client
                .get_event_selectors()
            ...
            {
                Ok(r) => {
                    let classic: Vec<String> = r
                        .event_selectors()
                        .iter()
                        .map(|es| {
                            let rw = es.read_write_type().map(|t| t.as_str()).unwrap_or("All");
                            let mgmt = es.include_management_events().unwrap_or(false);
                            let data: Vec<String> = es
                                .data_resources()
                                .iter()
            ...
            // Insight selectors
            let insight_selectors = match self
                .client
