...
impl CsvCollector for IamServiceAccountsCollector {
    ...
    fn headers(&self) -> &'static [&'static str] {
        &["project_id", "name", "email", "display_name", "description",
          "disabled", "oauth2_client_id", "unique_id"]
    }
        ...
        let rows = accounts
            ...
            .map(|sa| {
                vec![
                    ...
                    sa.get("oauth2ClientId").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                    sa.get("uniqueId").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                ]
            })
            .collect();
        Ok(rows)
