...
impl CsvCollector for CloudStoragePoliciesCollector {
    fn name(&self) -> &str { "GCP Cloud Storage Policies" }
    fn filename_prefix(&self) -> &str { "GCP_Storage_Policies" }
    ...
    async fn collect_rows(
        ...
        for bucket in &buckets {
            let name = bucket.get("name").and_then(|v| v.as_str()).unwrap_or("").to_owned();
