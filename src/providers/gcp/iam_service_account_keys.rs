...
impl CsvCollector for IamServiceAccountKeysCollector {
    ...
    fn headers(&self) -> &'static [&'static str] {
        &["project_id", "service_account", "key_name", "key_type",
          "key_origin", "key_algorithm", "valid_after", "valid_before", "disabled"]
    }
    ...
    async fn collect_rows(
        ...
        for sa in &accounts {
            ...
            let sa_email = sa.get("email").and_then(|v| v.as_str()).unwrap_or("").to_owned();
            let keys_url = format!(
            ...
            if let Some(keys) = body.get("keys").and_then(|k| k.as_array()) {
                for key in keys {
                    rows.push(vec![
                        ...
                        key.get("keyOrigin").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                        ...
                        key.get("disabled").and_then(|v| v.as_bool()).unwrap_or(false).to_string(),
                    ]);
                }
            }
        }
