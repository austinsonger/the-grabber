...
impl CsvCollector for CloudSqlBackupsCollector {
    fn name(&self) -> &str { "GCP Cloud SQL Backups" }
    fn filename_prefix(&self) -> &str { "GCP_Cloud_SQL_Backups" }
    fn headers(&self) -> &'static [&'static str] {
        &["project_id", "instance", "id", "status", "type",
          "start_time", "end_time", "backup_kind"]
    }
    ...
    async fn collect_rows(
        ...
        for inst in &instances {
            let inst_name = inst.get("name").and_then(|v| v.as_str()).unwrap_or("").to_owned();
            ...
            for backup in &backups {
                rows.push(vec![
                    ...
                    inst_name.clone(),
                    backup.get("id").and_then(|v| v.as_i64()).map(|i| i.to_string()).unwrap_or_default(),
                    backup.get("status").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                    backup.get("type").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                    backup.get("startTime").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                    backup.get("endTime").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                    backup.get("backupKind").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                ]);
            }
        }
        Ok(rows)
