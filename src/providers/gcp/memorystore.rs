...
impl CsvCollector for MemorystoreCollector {
    fn name(&self) -> &str { "GCP Memorystore" }
    fn filename_prefix(&self) -> &str { "GCP_Memorystore" }
    fn headers(&self) -> &'static [&'static str] {
        &["project_id", "name", "location", "tier", "memory_size_gb",
          "redis_version", "state", "host", "port",
          "auth_enabled", "transit_encryption", "create_time"]
    }
    ...
    async fn collect_rows(
        ...
        let rows = instances.iter().map(|inst| {
            ...
            vec![
                ...
                inst.get("memorySizeGb").and_then(|v| v.as_i64()).map(|i| i.to_string()).unwrap_or_default(),
                inst.get("redisVersion").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                inst.get("state").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                inst.get("host").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                inst.get("port").and_then(|v| v.as_i64()).map(|i| i.to_string()).unwrap_or_default(),
                auth,
                tls,
