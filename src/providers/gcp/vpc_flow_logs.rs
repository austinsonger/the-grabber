...
impl CsvCollector for VpcFlowLogsCollector {
    fn name(&self) -> &str { "GCP VPC Flow Logs" }
    fn filename_prefix(&self) -> &str { "GCP_VPC_Flow_Logs" }
    fn headers(&self) -> &'static [&'static str] {
        &["project_id", "region", "subnetwork", "network",
          "flow_logs_enabled", "aggregation_interval", "flow_sampling",
          "metadata", "ip_cidr_range"]
    }
    ...
    async fn collect_rows(
        ...
        if let Some(items) = body.get("items").and_then(|v| v.as_object()) {
            for (region_key, region_val) in items {
                ...
                if let Some(subnets) =
                    region_val.get("subnetworks").and_then(|v| v.as_array())
                {
                    for subnet in subnets {
                        ...
                        rows.push(vec![
                            ...
                            region.clone(),
                            subnet.get("name").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                            network,
                            enabled,
                            aggregation,
                            sampling,
                            metadata,
                            subnet.get("ipCidrRange").and_then(|v| v.as_str()).unwrap_or("").to_owned(),
                        ]);
                    }
                }
            }
