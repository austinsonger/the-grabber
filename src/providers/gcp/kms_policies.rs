    ...
    async fn collect_rows(
        ...
        for ring in &rings {
            ...
            for key in &keys {
                ...
                let policy_url =
                    format!("https://cloudkms.googleapis.com/v1/{}:getIamPolicy", key_name);
                ...
                if let Some(bindings) = policy.get("bindings").and_then(|b| b.as_array()) {
                    for binding in bindings {
                        let role =
                            binding.get("role").and_then(|v| v.as_str()).unwrap_or("").to_owned();
                        if let Some(members) =
                            binding.get("members").and_then(|m| m.as_array())
                        {
