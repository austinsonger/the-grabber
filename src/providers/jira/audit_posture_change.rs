...
impl CsvCollector for JiraAuditPostureChangeCollector {
    fn name(&self) -> &str { "Jira Audit Posture Change" }
    fn filename_prefix(&self) -> &str { "Jira_Audit_Posture_Change" }
    fn headers(&self) -> &'static [&'static str] {
        &["Ticket", "Summary", "Status", "Trigger Indicator", "Created", "Resolved"]
    }
    async fn collect_rows(
        ...
        for i in issues {
            let trigger_indicator = i.extra.get("labels")
                .map(|v| v.to_string())
                .unwrap_or_default();
            rows.push(vec![
