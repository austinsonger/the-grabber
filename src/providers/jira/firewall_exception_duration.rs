...
impl CsvCollector for JiraFirewallExceptionDurationCollector {
    fn name(&self) -> &str { "Jira Firewall Exception Duration" }
    fn filename_prefix(&self) -> &str { "Jira_Firewall_Exception_Duration" }
    fn headers(&self) -> &'static [&'static str] {
        &[
            "Ticket", "Summary", "Status", "Requestor", "Approver",
            "Created", "Expiration Date", "Days Active",
        ]
    }
    async fn collect_rows(
        ...
        let jql = format!(
            "project = {} AND labels = fw-exception",
            self.project_key
        );
        let issues = self.client.jql_sla().search(&jql, &["labels", "priority", "duedate"]).await?;
        let mut rows = Vec::with_capacity(issues.len());
        for i in issues {
            let expiration_date = i.extra.get("duedate").and_then(|v| v.as_str()).unwrap_or("").to_string();
