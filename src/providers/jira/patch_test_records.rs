...
impl CsvCollector for JiraPatchTestRecordsCollector {
    fn name(&self) -> &str { "Jira Patch Test Records" }
    fn filename_prefix(&self) -> &str { "Jira_Patch_Test_Records" }
    fn headers(&self) -> &'static [&'static str] {
        &[
            "Ticket", "Summary", "Status", "Patch ID", "Test Result",
            "Tested By", "Created", "Resolved",
        ]
    }
    async fn collect_rows(
        ...
        for i in issues {
            let patch_id = i.extra.get("labels")
