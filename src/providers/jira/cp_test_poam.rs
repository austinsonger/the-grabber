...
impl CsvCollector for JiraCpTestPoamCollector {
    fn name(&self) -> &str { "Jira CP Test POAM" }
    fn filename_prefix(&self) -> &str { "Jira_CP_Test_POAM" }
    fn headers(&self) -> &'static [&'static str] {
        &[
            "Ticket", "Summary", "Status", "Owner", "Due Date", "Created",
            "Resolved",
        ]
    }
    async fn collect_rows(
        ...
        for i in issues {
            let due_date = i.extra.get("duedate").and_then(|v| v.as_str()).unwrap_or("").to_string();
            rows.push(vec![
