...
impl CsvCollector for JiraBaselineExceptionsCollector {
    fn name(&self) -> &str { "Jira Baseline Exceptions" }
    fn filename_prefix(&self) -> &str { "Jira_Baseline_Exceptions" }
    fn headers(&self) -> &'static [&'static str] {
        &[
            "Ticket", "Summary", "Status", "Owner", "Config Rule",
            "Created", "Resolved",
        ]
    }
    async fn collect_rows(
        ...
        for i in issues {
            let config_rule = i.extra.get("labels")
